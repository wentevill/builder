...
echo "> Building qxcr.xyz/faas/buildpacks-run-go:$TAG"
docker build --build-arg "from_image=go116run" -t "qxcr.xyz/faas/buildpacks-run-go:$TAG" - < "${DIR}/run.Dockerfile"
echo "> Building qxcr.xyz/faas/buildpacks-go116-build:$TAG"
docker build --build-arg "from_image=go116common" -t "qxcr.xyz/faas/buildpacks-go116-build:$TAG" -f "${DIR}/build.Dockerfile" "${TEMP}"