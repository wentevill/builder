...
echo "> Building wentevill/buildpacks-run-go:$TAG"
docker build --build-arg "from_image=go116run" -t "wentevill/buildpacks-run-go:$TAG" - < "${DIR}/run.Dockerfile"
echo "> Building wentevill/buildpacks-go116-build:$TAG"
docker build --build-arg "from_image=go116common" -t "wentevill/buildpacks-go116-build:$TAG" -f "${DIR}/build.Dockerfile" "${TEMP}"