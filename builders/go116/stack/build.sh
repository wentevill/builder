...
echo "> Building openfunction/buildpacks-run-go:$TAG"
docker build --build-arg "from_image=go116run" -t "openfunction/buildpacks-run-go:$TAG" - < "${DIR}/run.Dockerfile"
echo "> Building openfunction/buildpacks-go116-build:$TAG"
docker build --build-arg "from_image=go116common" -t "openfunction/buildpacks-go116-build:$TAG" -f "${DIR}/build.Dockerfile" "${TEMP}"